...
uint8_t MQTT_Parse_JsonData(uint8_t *json) {
    ...
    cJSON_Delete(cJSON_device);//释放内存
    return SET;
}
/*
 *4g��������������
...
uint8_t CAT1_Parse_ATComander(uint8_t *recvbuf)
{
        ...
        strcpy(substr,"v1/devices/me/attributes");
               if( (startp = strstr((char *)startp,substr) ) != NULL )
        {
                    leftp   = strstr((char *)startp,"{"); // 开始位置
                    rightp  = strstr((char *)startp,"}"); // 结束位置
                    if( (leftp != NULL) && (rightp != NULL))
                    {
                            strncpy((char *)OTA_Info.recv_buf,leftp,rightp-leftp+1); // 复制cjson字符串
                            printf("jsonbuf2=%s\n",OTA_Info.recv_buf);
                            MQTT_Parse_OTAData((uint8_t*)OTA_Info.recv_buf);// 解析OTA attributes
                            return SET;
                    }
                    //startp = startp + strlen(substr); // �ƶ�λ��
    ...
    if(strncmp(OTA_Info.fw_checksum,crcstr,strlen(OTA_Info.fw_checksum)) == 0 )
        {
                ...
                // 必须先废弃旧固件再激活新固件, 防止中途断电导致两个APP都为0xAAAAAAAA
                uint32_t update1_flag = 0x00000000;
                MyFlash_Save(APP_DownloadOld_Address + Application_Size - 4, (char *)&update1_flag,4);
                uint32_t update2_flag = 0xAAAAAAAA;
                MyFlash_Save(APP_DownloadNew_Address + Application_Size - 4, (char *)&update2_flag,4);
