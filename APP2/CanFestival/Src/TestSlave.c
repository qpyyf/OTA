...
{
  printf("value=%x\r\n",*(uint8_t*)table->pSubindex[bSubindex].pObject);//table->pSubindex 就是 TestSlave_Index2000，即这个索引下所有子索引的数组：
  /*
  * // *(uint8_t*)table->pSubindex[bSubindex].pObject
       │    │       │      │          │           │
       │    │       │      │          │           └── 6. void* 指针，指向 REG_HOLD_BUF[9]
       │    │       │      │          └── 5. 取第 10 个 subindex 结构体
       │    │       │      └── 4. 子索引数组 TestSlave_Index2000
       │    │       └── 3. indextable 结构体 (OD 条目 0x2000)
       │    └── 2. 转为 uint8_t*（因为数据是 uint8 类型）
       └── 1. 解引用，读出 REG_HOLD_BUF[9] 的实际值
   */
  Modify_SlavemodbusAddress_Flag = 1;
                    ...
                    subindex TestSlave_Index1A03[] = 
                     {
                       ...
                       { RW, uint32, sizeof (UNS32), (void*)&REG_HOLD_BUF[0] },
                       { RW, uint32, sizeof (UNS32), (void*)&REG_HOLD_BUF[1] },
                       { RW, uint32, sizeof (UNS32), (void*)&REG_HOLD_BUF[2] },
                       { RW, uint32, sizeof (UNS32), (void*)&REG_HOLD_BUF[3] },
                       { RW, uint32, sizeof (UNS32), (void*)&REG_HOLD_BUF[4] },
                       { RW, uint32, sizeof (UNS32), (void*)&REG_HOLD_BUF[5] },
                       { RW, uint32, sizeof (UNS32), (void*)&REG_HOLD_BUF[6] },
                       { RW, uint32, sizeof (UNS32), (void*)&REG_HOLD_BUF[7] }
                     };
